 ...
 *
 * The bit array (see `wheel.ts`) only holds candidates coprime to 6, so
 ...
 */
export function nthPrime(nth: number): number {
  ...
  // Sieving phase: mark composites for every prime p <= sqrt(limit).
  // We start crossing out at p^2 because all smaller multiples of p have
